...
export async function getSystemBundle(systemId) {
  ...
  const [system, stats, classes, trees, items, proficiencies, levels, levelTrees, equipmentSlots] = await Promise.all([
    ...
    listEquipmentSlots(systemId),
  ]);
  return { system, stats, classes, trees, items, proficiencies, levels, levelTrees, equipmentSlots };
}
...
// campanha — usada pela aba "Habilidades" (jogador e Mestre, mesa.html). Diferente de
// getSkillBonuses (que só junta o EFEITO numérico das esferas com modificador de status), esta
// devolve a habilidade inteira (nome/tipo/via/descrição), incluindo o Núcleo de cada árvore mesmo
// sem nenhuma linha em system_player_unlocks (ele é sempre desbloqueado — mesma regra que
// isNodeUnlocked() já usa em arvore.js). Hoje só existe UMA fonte de desbloqueio (árvore de
// habilidades); o campo `source` em cada item é o gancho pra somar outras fontes no futuro sem
// precisar mudar quem consome esta função.
export async function getUnlockedAbilities(campaignId, userId) {
  const campaign = await getCampaign(campaignId);
  if (!campaign?.system_id) return [];
  const trees = await listSystemTrees(campaign.system_id);
  if (!trees.length) return [];
  const perTree = await Promise.all(trees.map(async (tree) => {
